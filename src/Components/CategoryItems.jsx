...
const workoutData = [
  ...
  { id: 5, imageSource: moderate, numberOfExercises: 9, title: "Moderate" },
  { id: 6, imageSource: strength, numberOfExercises: 9, title: "Strength" },
  { id: 7, imageSource: toning, numberOfExercises: 9, title: "Toning" },
];

const CategoryItems = () => {
  const navigation = useNavigation();

  const handleExercisesPress = (intensity) => {
    navigation.navigate("CategoryExercise", { intensity });
  };

  const renderWorkoutItem = ({ item }) => (
    <TouchableOpacity onPress={() => handleExercisesPress(item.title)}>
      <ImageBackground
